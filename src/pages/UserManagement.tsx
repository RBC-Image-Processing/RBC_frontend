...
import { EditingUser } from '../types/index';
import HashLoader from 'react-spinners/HashLoader';
import CustomTextField from '../components/CustomTextField';

export type RoleId = '1' | '2' | '3' | '4';

// Update the Role interface
export interface Role {
  roleName: string;
  roleId: RoleId;
}

// Create a constant for available roles
export const AVAILABLE_ROLES: Array<{ id: RoleId; name: string }> = [
  { id: '4', name: 'Administrator' },
  { id: '2', name: 'Physician' },
  { id: '3', name: 'Radiologist' },
  { id: '1', name: 'Non-Specialist' },
];

const UserManagement: React.FC = () => {
  const [fullName, setfullName] = useState('');
  const [newUserEmail, setNewUserEmail] = useState('');
  const [newUserRole, setNewUserRole] = useState<RoleId>('2');
  ...
  const handleSaveFullName = async (userId: string, newFullName: string) => {
    try {
      await updateUser(userId, { fullName: newFullName });
      setIsUpdateSuccess(true);
      setEditingUser(null);
    } catch (error) {
                    ...
                    editingUser.userId === user.userId ? (
                      <Box display="flex" alignItems="center">
                        <CustomTextField
                          ...
                          onSave={(value) => {
                            handleSaveFullName(user.userId, value);
                          }}
                          onCancel={() => handleCancelEdit(true)}
                        />
                      </Box>
                    ) : (
                      <Box display="flex" alignItems="center">
                    ...
                    editingUser.userId === user.userId ? (
                      <Box display="flex" alignItems="center">
                        <CustomTextField
                          ...
                          onSave={(value) => {
                            handleSaveEmail(user.userId, value);
                          }}
                          onCancel={() => handleCancelEdit(true)}
                        />
                      </Box>
              ...
              <CardContent>
                ...
                >
                  <Select
                    value={user.Role.roleId}
                    onChange={(e) =>
                      handleRoleChange(user.userId, e.target.value as RoleId)
                    }
                    size="small"
                    variant="standard"
                    fullWidth
