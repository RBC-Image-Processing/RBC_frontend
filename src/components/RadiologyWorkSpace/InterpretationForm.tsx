...
//   onSubmit,
// }) => {
//   const [text, setText] = useState<string>(interpretation?.text || '');
//   const canEdit = !interpretation || interpretation.radiologistId === currentRadiologistId; // Check if the current user can edit
//   const theme = useTheme();
...
//             variant="contained"
//             startIcon={<Send size={18} />}
//             onClick={() => onSubmit(text)} // Calls onSubmit with the text from the textarea
//             fullWidth
//             sx={{
//               py: 1.5,
//               bgcolor: theme.palette.primary.main,
...
export const InterpretationForm: React.FC<InterpretationFormProps> = ({
  ...
  const { interpretations} = useInterpretation();
...
//             onClick={handleSubmit} // Calls handleSubmit
          ...
          >
            {interpretations?.length > 0 ? 'Update' : 'Submit'} Interpretation
          </Button>
        </Box>
      )}
    </Paper>
